...
getPuzzle('3').then((puzzle) => {
    console.log(`GetPuzzle().then(puzzle): ${puzzle}`)
}).catch((err) => {
...
getLocation().then((ioData) => {
    // console.log(`getLocation() City: ${ioData.city}, Region: ${ioData.region}, Country: ${ioData.country}`)
...
getCurrentCountry().then((country) => {
    console.log(`getCurrentCountry: ${country}`)
}).catch ((error) => {
    console.log(error)
})
***/